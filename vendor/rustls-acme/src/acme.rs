...
impl Account {
    ...
    pub fn tls_alpn_01<'a>(&self, challenges: &'a [Challenge], domain: String) -> Result<(&'a Challenge, CertifiedKey), AcmeError> {
        ...
        let token = match challenge.token.as_deref() {
            Some(token) => token,
            None => return Err(AcmeError::ChallengeMissingToken),
        };
    ...
    pub fn http_01<'a>(&self, challenges: &'a [Challenge]) -> Result<(&'a Challenge, String), AcmeError> {
        ...
        let key_auth = key_authorization(&self.key_pair, token)?;

...
if [[ "$b_synced" != "1" ]]; then
  ...
  echo "[debug] leader members" | tee -a "$RES"
  curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20101/cluster/members | tee -a "$RES" || true
  echo | tee -a "$RES"
  echo "[debug] follower members" | tee -a "$RES"
  curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20102/cluster/members | tee -a "$RES" || true
  echo | tee -a "$RES"
  echo "[debug] leader gossip stats" | tee -a "$RES"
  curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20101/cluster/gossip/stats | tee -a "$RES" || true
  echo | tee -a "$RES"
  echo "[debug] follower gossip stats" | tee -a "$RES"
  curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20102/cluster/gossip/stats | tee -a "$RES" || true
  echo | tee -a "$RES"
  echo "[debug] generation timeline after failure" | tee -a "$RES"
  for _ in $(seq 1 40); do
    a_now=$(curl -sS --noproxy "*" http://127.0.0.1:20101/v1/status | python3 -c 'import json,sys; print(json.load(sys.stdin)["generation"])' || echo "err")
    b_now=$(curl -sS --noproxy "*" http://127.0.0.1:20102/v1/status | python3 -c 'import json,sys; print(json.load(sys.stdin)["generation"])' || echo "err")
    echo "timeline a_gen=$a_now b_gen=$b_now" | tee -a "$RES"
    sleep 0.2
  done
  echo "[debug] node-a log tail" | tee -a "$RES"
  tail -n 120 "$A_LOG" | tee -a "$RES"
  echo "[debug] node-b log tail" | tee -a "$RES"
  tail -n 120 "$B_LOG" | tee -a "$RES"
  exit 1
fi
...
if [[ "$b_detected" != "1" ]]; then
  ...
  echo "[debug] offline detection timeline" | tee -a "$RES"
  for _ in $(seq 1 40); do
    curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20102/cluster/members | tee -a "$RES" || true
    echo | tee -a "$RES"
    curl -sS --noproxy "*" -H "Authorization: Bearer tkn" http://127.0.0.1:20102/cluster/gossip/stats | tee -a "$RES" || true
  ...
  echo "[debug] node-b log tail (offline fail)" | tee -a "$RES"
  tail -n 200 "$B_LOG" | tee -a "$RES"
  exit 1
fi

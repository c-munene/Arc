...
pkill -x pebble-challtestsrv >/dev/null 2>&1 || true
sleep 1
...
# sanity check pebble dir
curl -k -sS -m 5 https://localhost:14000/dir >/dev/null || {
  echo 'PEBBLE_DIR_UNREACHABLE'
  tail -n 80 tmp/acme_pebble_full/pebble.log || true
  exit 1
}
...
cat > tmp/acme_pebble_full/arc.json <<'JSON'
{
  ...
  "downstream_tls": {
    ...
    "acme": {
      ...
      "directory_ca_pem": "tmp/pebble/test/certs/localhost/cert.pem",
...
# managed cert placeholders for config compile
...
GWPID=$!
echo $GWPID > tmp/acme_pebble_full/gateway.pid
sleep 3
...
echo '=== CHECK 2: HTTP-01 VALIDATION EVIDENCE ==='
grep -Ei 'challenge|authz|order|finalize|valid' tmp/acme_pebble_full/pebble.log | tail -n 40 || true

echo '=== CHECK 3: ACME FILES ==='

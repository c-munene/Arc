...
WORK=$ROOT/tmp/wsl_arc_gateway_h1_status_c512_long
mkdir -p "$WORK/backend_runtime/logs"

cat > "$WORK/backend_nginx.conf" <<'NGINX'
worker_processes auto;
pid /mnt/c/Users/Shuakami_Projects/CommunityProject/Arc/tmp/wsl_arc_gateway_h1_status_c512_long/backend.pid;
events { worker_connections 8192; use epoll; }
http {
  access_log off;
  error_log /mnt/c/Users/Shuakami_Projects/CommunityProject/Arc/tmp/wsl_arc_gateway_h1_status_c512_long/backend_error.log warn;
  server {
...
wrk -t8 -c512 -d60s --latency -s "$ROOT/tmp/wrk_status.lua" http://127.0.0.1:18080/x | tee "$WORK/arc_status_60s.txt"
